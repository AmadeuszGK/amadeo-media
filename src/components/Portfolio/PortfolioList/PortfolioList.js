...
class PortfolioList extends Component {
  ...
  render() {
    if (this.props.filter === 'default')  { 
      ...
      return(PortfolioArr.map(item => item.type === "website" && (<PortfolioItem {...item} key={item.name} />)))
    }
    else if (this.props.filter === 'store')  { 
      return(PortfolioArr.map(item => item.type === "shop" && (<PortfolioItem {...item} key={item.name} />)))
    };
  }
}
